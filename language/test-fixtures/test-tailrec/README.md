...
1. **test1-two-param.sigil** - Classic two-parameter accumulator
   - Status: ✅ BLOCKED - "has 2 parameters"
   
2. **test2-three-param.sigil** - Three parameters
   - Status: ✅ BLOCKED - "has 3 parameters"
   
3. **test3-list-param.sigil** - List encoding multiple values `[n,acc]`
   - Status: ✅ BLOCKED - "collection-type parameter"
   
4. **test4-tuple-param.sigil** - Tuple encoding `(n,acc)`
   - Status: ⊘ SKIPPED - Tuple types not yet implemented in parser
   
5. **test5-record-two-fields.sigil** - Record with 2 fields `{n:ℤ,acc:ℤ}`
   - Status: ✅ BLOCKED - "collection-type parameter"
   
6. **test6-record-three-fields.sigil** - Record with 3 fields
   - Status: ✅ BLOCKED - "collection-type parameter"
   
7. **test8-helper.sigil** - Helper function pattern (NOW ALLOWED)
   - Status: ✅ ALLOWED - Helper ban removed, utilities are allowed
   
8. **test9-cps.sigil** - Continuation Passing Style
   - Status: ✅ BLOCKED - "returns a function type"
   
9. **test10-map-param.sigil** - Map encoding state
   - Status: ⊘ SKIPPED - Map literals not yet implemented in parser
   
10. **test11-nested-list.sigil** - Nested list `[[n,acc]]`
    - Status: ✅ BLOCKED - "collection-type parameter"

### ✅ Tests that MUST be allowed (2/2 implemented)

11. **test7-record-one-field-ok.sigil** - Single-field record (not encoding multiple values)
    - Status: ✅ ALLOWED - Compiles successfully
    
12. **test12-valid-canonical.sigil** - Standard canonical recursion
    - Status: ✅ ALLOWED - Compiles successfully
...
**Enforcement: 100%** - There is exactly ONE way to write recursive functions in Sigil.
