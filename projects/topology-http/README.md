...
```bash
cargo run -q -p sigil-cli --manifest-path language/compiler/Cargo.toml -- run projects/topology-http/src/main.sigil
cargo run -q -p sigil-cli --manifest-path language/compiler/Cargo.toml -- run projects/topology-http/src/getClient.sigil --env local
```
